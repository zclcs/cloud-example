...
import com.zclcs.common.redis.starter.enums.BitMapModel;
...
public class RedisService {
     ...
     */
    public static RedisService use(RedisTemplate<String, Object> redisTemplate, RedissonClient redissonClient) {
    ...
    public <T> void set(CacheKey cacheKey, T value, Function<T, byte[]> mapper) {
        redisTemplate.execute((RedisCallback<Boolean>) redis -> {
            ...
            if (timeout == null) {
                return redis.stringCommands().set(rawKey, rawValue);
            } else {
                return redis.stringCommands().set(rawKey, rawValue, Expiration.from(timeout), RedisStringCommands.SetOption.ifPresent());
            }
        });
    }
    ...
    public void scanBytes(String pattern, @Nullable Long count, Consumer<byte[]> consumer) {
        redisTemplate.execute((RedisCallback<Object>) redis -> {
            ...
            try (Cursor<byte[]> cursor = redis.keyCommands().scan(builder.build())) {
    ...
    public void sScanBytes(String key, String pattern, @Nullable Long count, Consumer<byte[]> consumer) {
        redisTemplate.execute((RedisCallback<Object>) redis -> {
            ...
            try (Cursor<byte[]> cursor = redis.setCommands().sScan(keySerialize(key), builder.build())) {
                cursor.forEachRemaining(consumer);
            }
            return null;
        });
    ...
    public void mSet(Object... keysValues) {
        ...
        HashMap<Object, Object> map1 = CollectionUtil.toMap(keysValues);
        for (Map.Entry<Object, Object> objectObjectEntry : map1.entrySet()) {
            map.put((String) objectObjectEntry.getKey(), objectObjectEntry.getValue());
        }
    ...
    public Long getCounter(String key, long seconds, LongSupplier loader) {
        return redisTemplate.execute((RedisCallback<Long>) redis -> {
            ...
            byte[] value = redis.stringCommands().get(keyBytes);
            long longValue;
            if (value != null) {
                longValue = Long.parseLong(new String(value, StandardCharsets.UTF_8));
            } else {
                longValue = loader.getAsLong();
                redis.stringCommands().setEx(keyBytes, seconds, String.valueOf(longValue).getBytes());
            }
            return longValue;
        });
    }
    ...
    public Long getHCounter(String key, Object field, LongSupplier loader) {
        return redisTemplate.execute((RedisCallback<Long>) redis -> {
            ...
            byte[] value = redis.hashCommands().hGet(keyBytes, fieldBytes);
            long longValue;
            if (value != null) {
                longValue = Long.parseLong(new String(value, StandardCharsets.UTF_8));
            } else {
                longValue = loader.getAsLong();
                redis.hashCommands().hSet(keyBytes, fieldBytes, String.valueOf(longValue).getBytes());
            }
            return longValue;
        });
    }
    ...
    public Long bitCount(String key, long start, long end, BitMapModel model) {
        return redisTemplate.execute((RedisCallback<Long>) redis ->
                (Long) redis.execute(String.valueOf(RedisCommand.BITCOUNT), keySerialize(key),
