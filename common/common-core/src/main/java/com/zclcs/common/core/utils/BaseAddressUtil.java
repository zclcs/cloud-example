...
public abstract class BaseAddressUtil {
    ...
    public static String getCityInfo(String ip) {
        ...
        try {
            String dbPath = Objects.requireNonNull(BaseAddressUtil.class.getResource("/ip2region/ip2region.db")).getPath();
            File file = new File(dbPath);
            if (!file.exists()) {
                String tmpDir = System.getProperties().getProperty(MyConstant.JAVA_TEMP_DIR);
                dbPath = tmpDir + "ip.db";
                file = new File(dbPath);
                try (InputStream resourceAsStream = BaseAddressUtil.class.getClassLoader().getResourceAsStream("classpath:ip2region/ip2region.db")) {
