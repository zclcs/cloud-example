...
public class CanalGeneratorQueueHandler {
    ...
    @RabbitListener(bindings = {
            @QueueBinding(
                    value = @Queue(value = RabbitConstant.CANAL_GENERATOR_QUEUE, durable = "true", arguments = {
                            @Argument(name = RabbitConstant.DLX_EXCHANGE_PRE, value = RabbitConstant.DLX_EXCHANGE),
                            @Argument(name = RabbitConstant.DLX_ROUTE_KEY_PRE, value = RabbitConstant.CANAL_GENERATOR_DLX_ROUTE_KEY)
                    }),
                    exchange = @Exchange(value = RabbitConstant.CANAL_EXCHANGE),
                    key = RabbitConstant.CANAL_GENERATOR_ROUTE_KEY
            )
    })
    ...
    @RabbitListener(bindings = {
            @QueueBinding(
                    value = @Queue(value = RabbitConstant.CANAL_GENERATOR_DLX_QUEUE, durable = "true"),
                    exchange = @Exchange(value = RabbitConstant.DLX_EXCHANGE),
                    key = RabbitConstant.CANAL_GENERATOR_DLX_ROUTE_KEY
            )
    })
