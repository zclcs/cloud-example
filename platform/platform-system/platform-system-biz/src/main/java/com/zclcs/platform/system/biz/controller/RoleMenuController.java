...
public class RoleMenuController {
    ...
    @Inner
    public BaseRsp<List<Long>> findByRoleId(@PathVariable Long roleId) {
        List<Long> ids = roleMenuService.lambdaQuery().eq(RoleMenu::getRoleId, roleId).list().stream().map(RoleMenu::getMenuId).collect(Collectors.toList());
