...
public class UserDataPermissionController {
    ...
    public BaseRsp<List<Long>> findByUserId(@PathVariable Long userId) {
        List<Long> ids = userDataPermissionService.lambdaQuery().eq(UserDataPermission::getUserId, userId).list().stream().map(UserDataPermission::getDeptId).collect(Collectors.toList());
