...
@Slf4j
@Component
@RabbitListener(queues = "${my.rabbit.mq.direct-queues.systemLog.queue-name}", concurrency = "5")
public class SystemLogQueueHandler {
    ...
    public void directHandlerManualAck(String messageStruct, Message message, Channel channel) {
        ...
        try {
            log.debug("处理系统日志，手动ACK，接收消息：{}", messageStruct);
            LogAo logAo = objectMapper.readValue(messageStruct, LogAo.class);
