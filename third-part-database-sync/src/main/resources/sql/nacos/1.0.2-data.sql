...
call insert_or_update(database(), 'config_info',
                          ...
                          'data_id = values(data_id), tenant_id = values(tenant_id)');//
...
call insert_or_update(database(), 'config_info',
                      'data_id, group_id, content, md5, gmt_create, gmt_modified, src_user, src_ip, app_name,
                      tenant_id, c_desc, c_use, effect, type, c_schema, encrypted_data_key',
                      '("rabbitmq.yaml", "DEFAULT_GROUP", "spring:\n  rabbitmq:\n    host: ${RABBIT_MQ_HOST:127.0.0.1}\n    port: ${RABBIT_MQ_PORT:5672}\n    username: ${RABBIT_MQ_USERNAME:root}\n    password: ${RABBIT_MQ_PASSWORD:123456}\n    virtual-host: /\n    # 手动提交消息\n    listener:\n      simple:\n        #消息必须手动确认\n        acknowledge-mode: manual\n        #限制每次发送一条数据。\n        prefetch: 1\n    publisher-returns: true\n    publisher-confirm-type: simple\nmy:\n  rabbit:\n    mq:\n      enable: true\n      dlx-exchange: ${spring.cloud.nacos.config.namespace}.exchange.dlx\n      direct-exchange: ${spring.cloud.nacos.config.namespace}.exchange.direct\n      system-log-queue: ${spring.cloud.nacos.config.namespace}-system-log\n      system-log-queue-binding: ${spring.cloud.nacos.config.namespace}.system.log\n      system-login-log-queue: ${spring.cloud.nacos.config.namespace}-system-login-log\n      system-login-log-binding: ${spring.cloud.nacos.config.namespace}.system.login.log\n      system-rate-limit-log-queue: ${spring.cloud.nacos.config.namespace}-system-rate-limit-log\n      system-rate-limit-log-binding: ${spring.cloud.nacos.config.namespace}.system.rate.limit.log\n      system-block-log-queue: ${spring.cloud.nacos.config.namespace}-system-block-log\n      system-block-log-binding: ${spring.cloud.nacos.config.namespace}.system.block.log\n      system-route-log-queue: ${spring.cloud.nacos.config.namespace}-system-route-log\n      system-route-log-queue-binding: ${spring.cloud.nacos.config.namespace}.system.route.log",
