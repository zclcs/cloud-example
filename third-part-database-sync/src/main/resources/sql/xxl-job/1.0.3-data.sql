...
',
                          '(select id from xxl_job_group where app_name = "{{NACOS_NAMESPACE}}-platform-system"),"刷新限流配置和黑名单配置",now(),now(),"zclcs","","CRON","0 0/30 * * * ?","FIRE_ONCE_NOW","ROUND","refreshBlackListAndRateLimitRules","","DISCARD_LATER",0,2,"BEAN","","GLUE代码初始化",now(),"",1,0,0',
...
glue_type,glue_source,glue_remark,glue_updatetime,child_jobid,trigger_status,trigger_last_time,trigger_next_time
',
                          '(select id from xxl_job_group where app_name = "{{NACOS_NAMESPACE}}-platform-system"),"定时清理日志表",now(),now(),"zclcs","","CRON","0 0 0 1/1 * ?","FIRE_ONCE_NOW","ROUND","cleanLog","5000","DISCARD_LATER",0,2,"BEAN","","GLUE代码初始化",now(),"",1,0,0',
