...
call insert_if_not_exists(database(), 'xxl_job_group',
...
call insert_if_not_exists(database(), 'xxl_job_info',
                          'job_group,job_desc,add_time,update_time,author,alarm_email,schedule_type,schedule_conf,
misfire_strategy,executor_route_strategy,executor_handler,executor_param,executor_block_strategy,executor_timeout,executor_fail_retry_count,glue_type,
glue_source,glue_remark,glue_updatetime,child_jobid,trigger_status,trigger_last_time,trigger_next_time',
                          '(select id from xxl_job_group where app_name = "{{NACOS_NAMESPACE}}-platform-system"),"刷新限流配置和黑名单配置",now(),
now(),"zclcs","","CRON","0 0/30 * * * ?","DO_NOTHING","FIRST","refreshBlackListAndRateLimitRules","","SERIAL_EXECUTION",0,0,"BEAN",
"","GLUE代码初始化",now(),"",1,0,0',
                          'job_group=(select id from xxl_job_group where app_name = "{{NACOS_NAMESPACE}}-platform-system")
                          and executor_handler="refreshBlackListAndRateLimitRules"');