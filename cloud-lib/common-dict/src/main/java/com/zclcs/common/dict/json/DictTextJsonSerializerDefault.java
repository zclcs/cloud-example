...
import com.zclcs.common.core.enums.DictEnum;
...
import com.zclcs.common.dict.utils.DictPropertiesUtil;
...
public class DictTextJsonSerializerDefault extends JsonSerializer<Object> {
    ...
    public void serialize(@Nullable final Object fieldValue, final JsonGenerator gen, final SerializerProvider serializers) throws IOException {
        ...
        if (hasDictType) {
            ...
            logger.warn("{}#{} @DictText annotation not set name value", beanClass, beanFieldName);
        }
    }
    ...
    protected String getDictTypeByTypeKeyHandler(final Object bean, String dictValue) {
        ...
        if (dictTypeKeyHandler == null) {
            dictTypeKeyHandler = DictPropertiesUtil.getBean(factoryClass);
            if (dictTypeKeyHandler == null) {
                try {
                    dictTypeKeyHandler = factoryClass.getDeclaredConstructor().newInstance();
        ...
        return dictTypeKeyHandler.getDictName(bean, beanFieldName, dictValue, dictText);
    }
    ...
    protected void writeFieldValue(JsonGenerator gen, @Nullable Object fieldValue, Object dictValueText) throws IOException {
        ...
        if (dictText.mapValue().getValue(DictPropertiesUtil::isMapValue)) {
    ...
    public boolean isReplaceValue() {
        return dictText.replace().getValue(DictPropertiesUtil::isReplaceValue);
    }
    ...
    private void writeFieldValue(@Nullable Object fieldValue, JsonGenerator gen) throws IOException {
        if (DictPropertiesUtil.isRawValue()) {
    ...
    protected Object defaultNullableValue(Object dictValueText) {
        if (dictText.nullable().getValue(DictPropertiesUtil::isTextValueDefaultNull)) {
