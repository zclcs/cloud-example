...
import com.zclcs.cloud.lib.aop.annotation.RedisRateLimiter;
import com.zclcs.cloud.lib.aop.spel.MyExpressionEvaluator;
...
public class RedisRateLimiterAspect implements ApplicationContextAware {
     ...
     * 表达式处理
     */
    private final MyExpressionEvaluator evaluator = new MyExpressionEvaluator();
    /**
     * redis 限流服务
     */
    private final DefaultRateLimiterClient rateLimiterClient;
    private ApplicationContext applicationContext;

    /**
     * AOP 环切 注解 @RedisRateLimiter
     */
    @Around("@annotation(limiter)")
    public Object aroundRateLimiter(ProceedingJoinPoint point, RedisRateLimiter limiter) throws Throwable {
        // 服务内部调用不限流
        boolean valid = SaSameUtil.isValid(SaSameUtil.getToken());
        if (valid) {
            return point.proceed();
        }
        String limitKey = limiter.value();
        Assert.hasText(limitKey, "@RedisRateLimiter value must have length; it must not be null or empty");
        // el 表达式
    ...
    private String evalLimitParam(ProceedingJoinPoint point, String limitParam) {
        ...
        context.setVariable("requestIpAddress", WebUtil.getHttpServletRequestIpAddress());
